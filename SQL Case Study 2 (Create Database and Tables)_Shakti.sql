...
--1. List all the cells in which we have customers who have bought cellphones from 2005 to till today 

select*from DIM_CUSTOMER ,DIM_DATE,FACT_TRANSACTIONS WHERE YEAR >= 2005 ORDER BY YEAR

--2.	What state in the US is buying more 'Samsung' cell phones?

SELECT top 1 DIM_LOCATION.Country,
DIM_LOCATION.State,
DIM_MANUFACTURER.Manufacturer_Name,
SUM(FACT_TRANSACTIONS.Quantity) AS Quantity

FROM DIM_LOCATION LEFT JOIN FACT_TRANSACTIONS ON DIM_LOCATION.IDLocation = FACT_TRANSACTIONS.IDLocation
				
				RIGHT JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDModel = DIM_MODEL.IDModel
				
				RIGHT JOIN DIM_MANUFACTURER ON DIM_MODEL.IDManufacturer = DIM_MANUFACTURER.IDManufacturer

WHERE Country = 'US' and  DIM_MANUFACTURER.Manufacturer_Name = 'Samsung'
GROUP BY DIM_LOCATION.Country,
DIM_LOCATION.State,DIM_MANUFACTURER.Manufacturer_Name
ORDER BY MAX(FACT_TRANSACTIONS.Quantity) 
...
COUNT(FACT_TRANSACTIONS.IDCustomer) AS [Count of Trans]
from DIM_LOCATION LEFT JOIN FACT_TRANSACTIONS ON DIM_LOCATION.IDLocation = FACT_TRANSACTIONS.IDLocation 
					RIGHT JOIN DIM_MODEL ON FACT_TRANSACTIONS.IDModel = DIM_MODEL.IDModel
...
select dm.IDModel,avg(cast(t.TotalPrice as float)/Quantity) as avg_price from dim_model dm  inner join fact_transactions t  on dm.idmodel=t.IDModel
where dm.IDManufacturer in (select top 5 dm.idmanufacturer from dim_model dm inner join FACT_TRANSACTIONS t on dm.IDModel=t.IDModel 
...
--6.	List the names of the customers and the average amount spent in 2009, where the average is higher than 500
 ...
 select DIM_CUSTOMER.IDCustomer,
 DIM_CUSTOMER.Customer_Name,
 YEAR(FACT_TRANSACTIONS.Date) as [Year],
 AVG(TotalPrice) as [Avg Spent]
 from DIM_CUSTOMER left join FACT_TRANSACTIONS on  DIM_CUSTOMER.IDCustomer =  FACT_TRANSACTIONS.IDCustomer 
--			left join DIM_MODEL on DIM_MODEL.IDModel = FACT_TRANSACTIONS.IDModel
 WHERE YEAR(FACT_TRANSACTIONS.Date) = '2009' 
 group by  DIM_CUSTOMER.IDCustomer,DIM_CUSTOMER.Customer_Name,TotalPrice,Date
...
select TOP 5 DIM_MODEL.IDModel , Model_Name,
max(Quantity) 
from DIM_MODEL left join FACT_TRANSACTIONS on DIM_MODEL.IDModel = FACT_TRANSACTIONS.IDModel
				LEFT JOIN DIM_DATE ON DIM_DATE.DATE = FACT_TRANSACTIONS.Date
where YEAR(FACT_TRANSACTIONS.Date) BETWEEN '2008' and '2010'
GROUP BY DIM_MODEL.IDModel, Model_Name
...
Select * from 
  (Select  top 2 dmr.Manufacturer_Name
  from
  DIM_MANUFACTURER dmr join DIM_MODEL dm on Dmr.IDManufacturer = Dm.IDManufacturer join FACT_TRANSACTIONS ft on Ft.IDModel = Dm.IDModel
  Where Year(ft.Date)= 2009
  Group by dmr.Manufacturer_Name
  ...
  Group by dmr.Manufacturer_Name
  Order by Sum(ft.TotalPrice) desc)as t1
...
WHERE YEAR(FACT_TRANSACTIONS.Date) = '2010' and YEAR(FACT_TRANSACTIONS.Date) <> '2009'
GROUP BY DIM_MANUFACTURER.Manufacturer_Name,YEAR(FACT_TRANSACTIONS.Date),DIM_MODEL.IDModel
...
SELECT top 100  DIM_CUSTOMER.IDCustomer,DIM_CUSTOMER.Customer_Name,
year(Date) AS [ YEAR] ,
AVG(FACT_TRANSACTIONS.TotalPrice) AS [AVG. SPENT],
AVG(Quantity) AS [AVG. QUANTITY]
FROM DIM_CUSTOMER LEFT join  FACT_TRANSACTIONS on DIM_CUSTOMER.IDCustomer = FACT_TRANSACTIONS.IDCustomer
group by DIM_CUSTOMER.IDCustomer,DIM_CUSTOMER.Customer_Name,year(Date)
